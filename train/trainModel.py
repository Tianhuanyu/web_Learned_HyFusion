...
def main_kalman_net():
    ...
    pipeline_instance = Pipeline(train_indices=list(range(0, 11)), args=args)
    ...
    KF_model = KalmanNet(system_model=task_model, initial_state=init_state,
                         initial_covariance=init_covariance, args=args, dt=dt_value)\
                         .to(task_model.device)
    
    pipeline_instance.set_nn_model(KF_model)
    
    # Optionally, provide a checkpoint path or use None
    pipeline_instance.train_network("best_model_0.001_150_0.0001_seq20_KFNET.pth")
